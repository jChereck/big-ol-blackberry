...
// constructor
SymbolNumMap::SymbolNumMap(int startnum, string defaultValue) :
    startNum(startnum), defaultMissing(defaultValue), next(startnum){
};
...
int SymbolNumMap::getNum(string s) {
    if (mapdata.find(s) == mapdata.end()) {
        return -1;
    }
    return mapdata.find(s)->second;
...
int SymbolNumMap::getNum(const char *s) {
    if (s==NULL) {
...
string SymbolNumMap::getStr(int n) {
    if (maprev.find(n) == maprev.end()) {
        return defaultMissing;
    }
    return maprev.find(n)->second;
}


// get the string associate with the double or error for one of several reasons
string SymbolNumMap::getStr(double x) {
    int n = x;
    if (n != x) {
    ...
    else if (maprev.find(n) == maprev.end()) {  
...
SymbolNumMap Matrix::readAux(bool labeled, bool transpose)
{
    int r, c;
    int numread;
    SymbolNumMap label(10000);
    ...
    for (int r=0; r<maxr; r++) {
        for (int c=0; c<maxc; c++) {
            if (labeled && c==0) {
                scanf("%s", buffer);
                m[r][c] = label.add(buffer);   // effectively numeric pointer to the label
            }
            else {

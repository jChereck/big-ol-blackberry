...
private:
    ...
    map<string, int> mapdata;   // map string to int
    map<int, string> maprev;    // map int to string

public:
    SymbolNumMap(int startnum=0, string defaultValue="NONE");
...
public:
    ...
    string getStr(int n);        // get the string associated with n or return default string
    string getStr(double x);     // get the string associated with x but test x and error if not valid
    void clear();
};
...
private:
    SymbolNumMap readAux(bool labeled, bool transpose);    // general read routine both labeled and un
